...
`GrepCommand` - считывает данные из файла, использует regex для поиска паттерна.

Для реализации парсинга аргументов командной строки было принято решение выбрать крейт `clap`, также
рассматривался крейт `structopt`, однако первый подошел по следующим причинам:

`clap`:
* имеет бОльшее сообщество контрибьютуров, а соответственно и поддержку
* имеет бОльшее количество фич
